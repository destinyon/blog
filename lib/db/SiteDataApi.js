...
function getSiteInfo({ collection, block, rawMetadata, NOTION_CONFIG }) {
  ...
  const configuredIcon = NOTION_CONFIG?.AVATAR || BLOG.AVATAR
  const defaultIcon = configuredIcon || '/avatar.svg'
  ...
  if (!configuredIcon && collection?.icon) {
    icon = compressImage(mapImgUrl(collection.icon, collection, 'collection'))
  }
