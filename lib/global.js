import { APPEARANCE, AVATAR, LANG, NOTION_PAGE_ID, THEME } from '@/blog.config'
import {
  THEMES,
  getThemeConfig,
  initDarkMode,
...
export function GlobalContextProvider(props) {
  ...
  const siteInfoWithConfig = useMemo(() => {
    const configuredAvatar = NOTION_CONFIG?.AVATAR || AVATAR
    if (!configuredAvatar) {
      return siteInfo
    }
    return {
      ...siteInfo,
      icon: configuredAvatar
    }
  }, [NOTION_CONFIG?.AVATAR, siteInfo])
  ...
  const [theme, setTheme] = useState(THEME) // 默认博客主题
  ...
  const contextValue = useMemo(() => ({
    ...
    setTheme,
    switchTheme,
    siteInfo: siteInfoWithConfig,
    categoryOptions,
    tagOptions
  }), [isLiteMode, isLoaded, isSignedIn, user, fullWidth, NOTION_CONFIG, THEME_CONFIG, toggleDarkMode, onLoading, setOnLoading, lang, changeLang, locale, updateLocale, isDarkMode, updateDarkMode, theme, setTheme, switchTheme, siteInfoWithConfig, categoryOptions, tagOptions])
